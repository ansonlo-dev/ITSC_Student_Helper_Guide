# 欢迎加入

欢迎加入岭南大学 ITSC 学生助理团队。本网站是学生助理迎新简介的文字版：团队的
工作范围、各项职务的执行方法，以及遇到超出职权范围的情况时应该联系谁。

## 我们的工作

学生助理支援 **SUS（System & User Service，系统及用户服务）**。工作分为两大部分：

| 柜台（面向用户） | 维护／检查 |
| --- | --- |
| 服务热线 | 教室 |
| 用户账户 | 打印机 |
| 器材外借 | 电脑室 |
| 网络 | 宿舍 |
| 双重验证（2FA） | |
| 其他临时事项 | |

## 学生助理团队

| 姓名 | 职位 | 电子邮件 | 分机 |
| --- | --- | --- | --- |
| HUI Felix | Team Leader | felixhui@ln.edu.hk | 2616-8407 |
| Paul Leung | Student Helper Admin | paulleung@ln.edu.hk | 2616-7393 |
| LAU Daniel | Team Manager | laumf@ln.edu.hk | 2616-7585 |
| Anson | Senior Student Helper（现场问题核实） | chikonglo@ln.hk | — |
| Jason | Senior Student Helper（文件记录） | yuxiangyin@ln.hk | — |

各类问题的专责联系人请见[联系与升级处理](/zh-CN/reference/contacts)。

## 沟通渠道

- **WhatsApp 群组** —— 日常信息、请假，以及接手其他助理放弃的班次。
- **Microsoft Teams —“ITSC Student Helpers”** —— 检查表及参考文件，按频道
  分类：General、1 Morning Check、2a Printer Check、2b Paper Stock、
  3 Computer Lab Check、4 Lecture Room Check，以及 Hostel Clinic Checklist。

## 如何使用本指南

- **第一次值班值班前** —— 请阅读[第一次值班值班](/zh-CN/guide/first-shift)及
  [工作注意事项](/zh-CN/guide/code-of-conduct)。
- **值班期间** —— 打开[职务](/zh-CN/duties/)下的相关页面，或使用搜寻功能
  （按 `/`）。
- **遇到困难时** —— 先看[故障排查手册](/zh-CN/tips/troubleshooting)，然后询问
  值班职员。

::: tip 不确定就发问
任何不确定的事情，都应该询问全职职员。**先找值班职员**，然后才找其他人。没有人
会介意你发问。
:::
